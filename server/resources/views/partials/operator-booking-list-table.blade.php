...
$showPendingActionsColumn = $bookingList === 'pending';
@endphp
...
<form id="booking-bulk-delete"
      method="POST"
      action="{{ route('operator.bookings.bulkDismiss') }}"
        ...
        <table class="console-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    @if($showPendingActionsColumn)
                    <td class="small">
                        @include('partials.operator-booking-pending-actions', [
                            'booking' => $booking,
                            'drivers' => $drivers,
                        ])
                    </td>
                    @endif
                    <td class="small">
                        @if($bookingList === 'pending' && $booking->isInOperatorPendingQueue())
                            @if($reason = $booking->operatorHelpReasonLabel())
                                <div class="text-danger fw-semibold mb-1">⚠ {{ $reason }}</div>
                            @endif
                            @if($booking->needs_operator_help_at)
                                <div class="cell-muted mb-1">Từ {{ $booking->needs_operator_help_at->format('H:i, d/m/Y') }}</div>
                            @endif
                        @endif
