...
<div class="driver-approval-actions {{ $compact ? 'driver-approval-actions--compact' : '' }}">
    ...
    <form method="POST"
        ...
        @csrf
        <label class="form-label small mb-1" for="rejection_reason_{{ $driver->id }}">Lý do từ chối <span class="text-danger">*</span></label>
        <textarea name="rejection_reason"
                  id="rejection_reason_{{ $driver->id }}"
                  class="form-control form-control-sm @error('rejection_reason') is-invalid @enderror"
                  rows="{{ $compact ? 2 : 3 }}"
                  minlength="5"
                  maxlength="1000"
                  required
                  placeholder="Ví dụ: Ảnh giấy tờ không rõ, thiếu thông tin ngân hàng…">{{ old('rejection_reason') }}</textarea>
