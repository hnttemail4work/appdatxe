...
class BookingController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $seatNumbers = collect($validated['seat_numbers'])->map(fn ($seatNumber): string => trim((string) $seatNumber))->unique()->values()->all();
        ...
        $booking = DB::transaction(function () use ($validated, $seatNumbers, $request): Booking {
            ...
            $paymentPayload = $this->buildMockPaymentPayload($booking);
            ...
            PaymentTransaction::query()->create([
                ...
                'currency' => 'VND',
                'status' => 'pending',
                'transaction_ref' => $paymentPayload['reference'],
            ...
            return $booking->load(['schedule.route', 'schedule.vehicle', 'seatReservations', 'paymentTransactions']);
        });

        return response()->json([
            'message' => 'Booking created successfully.',
            'data' => $booking,
            'deposit_payload' => $this->buildMockPaymentPayload($booking),
        ], 201);
    }

    public function show(Booking $booking): JsonResponse
    {
        $booking->load(['customer', 'schedule.route', 'schedule.vehicle', 'seatReservations', 'paymentTransactions', 'audits.actor']);
    ...
    public function destroy(Request $request, Booking $booking): JsonResponse
    {
        ...
        DB::transaction(function () use ($booking, $request, $beforeState): void {
            ...
            $booking->paymentTransactions()->update(['status' => 'refunded']);
    ...
    public function depositPayload(Booking $booking): JsonResponse
    {
        return response()->json(['data' => $this->buildMockPaymentPayload($booking)]);
    }
    ...
    public function confirmPayment(Request $request, Booking $booking): JsonResponse
    {
        ...
        DB::transaction(function () use ($booking, $request, $beforeState): void {
            $transaction = $booking->paymentTransactions()->latest()->first();

            if (! $transaction) {
                $transaction = $booking->paymentTransactions()->create([
                    'provider' => 'vietqr',
                    'amount' => $booking->deposit_amount,
                    'currency' => 'VND',
                    'status' => 'pending',
                    'transaction_ref' => 'TX-' . Str::upper(Str::random(12)),
                    'payload' => $this->buildMockPaymentPayload($booking),
                ]);
            }

            $transaction->update(['status' => 'succeeded', 'paid_at' => now()]);
    ...
    public function accept(Request $request, Booking $booking): JsonResponse
    {
        if ($booking->payment_status !== 'paid') {
            return response()->json(['message' => 'Deposit payment must be marked as paid before confirmation.'], 422);
        }
    ...
    private function buildMockPaymentPayload(Booking $booking): array
    {
        return [
            ...
            'vietqr' => [
                'bank_code' => 'VCB',
                'account_name' => 'APP DAT XE',
                'account_number' => '0000000000',
                'content' => 'Thanh toan coc ' . $booking->booking_reference,
            ],
            'momo' => [
                'payment_url' => url('/payments/mock/momo/' . $booking->id),
            ],
        ];
    }
