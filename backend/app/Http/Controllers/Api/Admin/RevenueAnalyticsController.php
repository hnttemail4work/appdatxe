...
class RevenueAnalyticsController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $grossRevenue = (float) Booking::query()
            ->whereBetween(DB::raw('COALESCE(confirmed_at, deposit_paid_at, created_at)'), [$startDate, $endDate])
        ...
        $operatorBreakdown = Booking::query()
            ...
            ->whereBetween(DB::raw('COALESCE(bookings.confirmed_at, bookings.deposit_paid_at, bookings.created_at)'), [$startDate, $endDate])
